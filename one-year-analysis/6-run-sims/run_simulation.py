...
# ===========================================================================
# PATH SETUP — where each stage reads its inputs
# ===========================================================================
#
# The pipeline reads from two different source trees depending on whether a
# full run (stages 1–4) or a partial run is executed:
#
#   Source tree — pre-generated data (used when skipping earlier stages)
#   ─────────────────────────────────────────────────────────────────────
#   _NSRDB_DIR   1-solar-power/NSRDB-raw/
#                  Raw NSRDB PSM3 hourly CSVs downloaded from
#                  https://nsrdb.nrel.gov/data-viewer (one file per year,
#                  2018–2024, site ID 4469509).  Only read by Stage 1.
#                  These files are large and not re-generated by this script.
#
#   _POWER_DIR   1-solar-power/gen-power/
#                  Hourly solar DC-power CSVs produced by solar_analysis.py
#                  for the REFERENCE array (15 panels × 405 W).  Stage 4
#                  always scales these proportionally when testing different
#                  panel counts — the raw files are never regenerated just
#                  because you change n_panels.  Stage 2 also reads from here
#                  when Stage 1 is skipped.
#
#   _ET_DIR      4-irrigation/et-data/
#                  Daily climate + ET0 CSVs produced by fetch_et_data.py
#                  (run that script once; it calls NSRDB + Open-Meteo APIs).
#                  Only read by Stage 3.
#
#   _IRR_DIR     4-irrigation/results/
#                  Weekly and daily irrigation schedule CSVs produced by
#                  irrigation_schedule.py (or by Stage 3 of this script).
#                  Only read by Stage 4 when Stage 3 is not included in the
#                  current run.
#
#   Output tree — written during this run
#   ──────────────────────────────────────
#   _OUT_ROOT    6-run-sims/output/
#                  Every run creates a tagged subfolder here.  Stages write
#                  gen-power/, battery-pump/, irrigation/, integrated/,
#                  and images-stage{N}/ subdirectories within that tag.
#
# ─────────────────────────────────────────────────────────────────────────────
# WHEN TO RUN A PARTIAL STAGE SELECTION
# ─────────────────────────────────────────────────────────────────────────────
#
#   --stages 1 2          Solar + greedy pump simulation only.
#                         Useful to estimate total annual pump-hours and check
#                         whether the hardware can run long enough, before
#                         committing to an irrigation schedule.
#
#   --stages 3 4          Irrigation scheduling + integrated analysis only.
#                         ★ The most common partial run. ★
#                         Use this when:
#                           • You want to test different panel counts or battery
#                             sizes without re-generating solar CSVs (Stage 1).
#                             Stage 4 scales the existing 15-panel reference
#                             CSVs internally — Stage 1 is not required.
#                           • Climate data already exists in _ET_DIR (i.e. you
#                             have already run fetch_et_data.py).
#                           • You just changed a crop, schedule, or irrigation
#                             parameter and need updated reliability numbers fast.
#                         Reads solar from _POWER_DIR (reference 15-panel CSVs).
#                         Reads ET data from _ET_DIR.
#
#   --stages 4            Integrated analysis only.
#                         Use when irrigation schedules already exist in _IRR_DIR
#                         and you only want to re-run the energy simulation
#                         (e.g. after changing battery_kwh or pump_power_kw).
#                         Reads solar from _POWER_DIR.
#                         Reads schedules from _IRR_DIR.
#
#   --sweep-panels ...    Automatically runs Stage 4 only across all
#   --sweep-battery ...   (panels, battery) combinations.  Stage 1 is NOT re-run
#                         because panel count is handled by the internal scale
#                         factor (n_panels / 15) inside integrated_analysis.py.
# ─────────────────────────────────────────────────────────────────────────────

_HERE       = os.path.dirname(os.path.abspath(__file__))
_ROOT       = os.path.abspath(os.path.join(_HERE, '..'))
_NSRDB_DIR  = os.path.join(_ROOT, '1-solar-power', 'NSRDB-raw')   # Stage 1 input
_POWER_DIR  = os.path.join(_ROOT, '1-solar-power', 'gen-power')   # Stage 2+4 input (reference 15-panel CSVs)
_ET_DIR     = os.path.join(_ROOT, '4-irrigation', 'et-data')      # Stage 3 input (from fetch_et_data.py)
_IRR_DIR    = os.path.join(_ROOT, '4-irrigation', 'results')      # Stage 4 input (pre-generated schedules)
_SYS_DIR    = os.path.join(_ROOT, '3-operating-hours-available', 'battery-pump')  # legacy; not read by this script
_OUT_ROOT   = os.path.join(_HERE, 'output')                        # all run outputs written here
